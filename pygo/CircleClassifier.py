...
class CircleClassifier(Classifier, DebugInfoProvider, Timing):
    def __init__(self, BOARD:GoBoard, size: int) -> None:
        ...
        for key in CircleClassifierDebugKeys:        
    ...
    def predict(self, img: B3CImage) -> GoBoardClassification:
        ...
        self.showDebug(CircleClassifierDebugKeys.DETECT0, img_detect0)
        self.showDebug(CircleClassifierDebugKeys.DETECT1, img_detect1)
        self.showDebug(CircleClassifierDebugKeys.DETECT2, img_detect2)
        self.showDebug(CircleClassifierDebugKeys.DETECT3, img_detect3)
        self.showDebug(CircleClassifierDebugKeys.DETECT4, img_detect4)
        self.showDebug(CircleClassifierDebugKeys.MASK, mask)
    ...
    def _detect_hidden_intersection(self, img):
        ...
        self.showDebug(CircleClassifierDebugKeys.GRID, hidden_corners)
        detections = []
        #for coord in self.BOARD.go_board_shifted.astype(int):
    ...
    def _detect_circle_detection_on_gradient(self, img):
        ...
        self.showDebug(CircleClassifierDebugKeys.BIN, bin_img)
        self.showDebug(CircleClassifierDebugKeys.CIRCLE, crcl)
        detections = []
        for coord in self.BOARD.go_board_shifted.astype(int):
    ...
    def _detect_blobb(self, img: B3CImage) -> GoBoardClassification:
        ...
        self.showDebug(CircleClassifierDebugKeys.IMG_B, img_b)
        self.showDebug(CircleClassifierDebugKeys.IMG_W, img_w)
        ...
        for img in img_masks:
            ...
            if self.debugStatus(CircleClassifierDebugKeys.Detected_Intensities):
        ...
        self.showDebug(CircleClassifierDebugKeys.Mask_Black, mask_b)
        self.showDebug(CircleClassifierDebugKeys.Mask_White, mask_w)
                ...
                for i in circles:
                    ...
                    if np.any(on_grid):
                        ...
                        if self.debugStatus(CircleClassifierDebugKeys.Detected_Intensities):
                            ...
                            self.showDebug(CircleClassifierDebugKeys.Detected_Intensities, self.img_debug)
        detections = np.array(detections)
